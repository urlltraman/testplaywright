...
test('TC30 Sub District 1 - 4 ', async ({ page }) => {
    // C1
    await page.waitForURL('/batch');
    await page.getByRole('button', { name: 'Upload' }).first().click();
...
test('TC30 Sub District 5 - 8 ', async ({ page }) => {
    // C5
    await page.waitForURL('/batch');
    await page.getByRole('button', { name: 'Upload' }).first().click();
    ...
    await expect.soft(page.getByLabel('Message')).toBeVisible();
    await expect.soft(page.getByLabel('Message'))
        .toHaveText("Messageข้อมูลบรรทัดที่ 1 column 'subdistrict' invalid special character ข้อมูลบรรทัดที่ 1 column 'subdistrict' invalid character. ");
    await page.waitForTimeout(500);
    ...
    await expect.soft(page.locator('.card-error')).toBeVisible();
    await expect.soft(page.locator('.card-error'))
        .toHaveText("errorInput Errorข้อมูลบรรทัดที่ 1 column 'subdistrict' invalid special character ข้อมูลบรรทัดที่ 1 column 'subdistrict' invalid character. Import FixesExport FixesEdit");
    await page.waitForTimeout(500);
