...
test('TC47 Overdue Flag  11 - 12 ', async ({ page }) => {
    ...
    await expect.soft(page.getByLabel('Message')).toBeVisible();
    await expect.soft(page.getByLabel('Message'))
        .toHaveText("Messageข้อมูลบรรทัดที่ 1 column 'receiveData' invalid overdueFlag ข้อมูลบรรทัดที่ 1 column 'receiveDate' instInfo.receiveDate must not be before instInfo.creationDate. [E07360] ข้อมูลบรรทัดที่ 1 column 'receiveDate' creationDate must not be over than receiveDate. [R51] ");
    await page.waitForTimeout(500);
...
test('TC50 Detail5,6,7,8  1 - 2 ', async ({ page }) => {
    // D5 C1
    await page.waitForURL('/batch');
    await page.getByRole('button', { name: 'Upload' }).first().click();
