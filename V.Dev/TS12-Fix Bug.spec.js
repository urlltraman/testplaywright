...
test('GSD-142 ', async ({ page }) => {
    ...
    await expect(page.getByText('CompanyGinkgo Soft Co., Ltd.Ginkgo Soft Co., Ltd. (Demo)')).toHaveText('CompanyGinkgo Soft Co., Ltd.Ginkgo Soft Co., Ltd. (Demo)');
...
test('GSD-154 ', async ({ page }) => {
    ...
    await page.waitForURL('/batch');
    await page.getByRole('button', { name: 'Upload' }).first().click();
    await page.locator('#file').setInputFiles('Data Files/Fix Bug/GSD-154/Auto_GSD-154.json');
    await page.getByRole('button', { name: 'Upload' }).click();
    await page.getByLabel('Upload Success').click();
    await expect(page.getByRole('heading', { name: 'Upload Success' })).toHaveText('Upload Success');
    await page.getByRole('button', { name: 'Close' }).click();
    ...
    await expect.soft(page.getByLabel('Message')).toBeVisible();
    await expect.soft(page.getByLabel('Message'))
        .toHaveText("Messageข้อมูลบรรทัดที่ 1 column 'arrayDetail' typeCode is equal to 1 and attType is equal to 2 and attActionType1 is equal to 1 or 2 therefore, attNumber1 and attAmount1 must be more than 0 [R56] ข้อมูลบรรทัดที่ 1 column 'receiveData' invalid overdueFlag ข้อมูลบรรทัดที่ 1 column 'actionType' typeCode is equal to 1,4,5,91,92,10,112,12,13,16,19 or 20; therefore, actionType must be empty. [R21] ");
    await page.waitForTimeout(500);
    ...
    await expect.soft(page.locator('.card-error')).toBeVisible();
    await expect.soft(page.locator('.card-error'))
        .toHaveText("errorInput Errorข้อมูลบรรทัดที่ 1 column 'arrayDetail' typeCode is equal to 1 and attType is equal to 2 and attActionType1 is equal to 1 or 2 therefore, attNumber1 and attAmount1 must be more than 0 [R56] ข้อมูลบรรทัดที่ 1 column 'receiveData' invalid overdueFlag ข้อมูลบรรทัดที่ 1 column 'actionType' typeCode is equal to 1,4,5,91,92,10,112,12,13,16,19 or 20; therefore, actionType must be empty. [R21] Import FixesExport FixesEdit");
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'Output/TS12-Fix Bug/GSD-154/03. GSD-154 Error Msg.png' });
    await page.getByRole('button', { name: 'Edit' }).click();
    await page.waitForTimeout(1500);
    await expect(page.getByText('typeCode is equal to 1,4,5,91,92,10,112,12,13,16,19 or 20; therefore, actionType must be empty. [R21]')).toHaveText('typeCode is equal to 1,4,5,91,92,10,112,12,13,16,19 or 20; therefore, actionType must be empty. [R21]');
    await expect(page.getByText('typeCode is equal to 1 and attType is equal to 2 and attActionType1 is equal to 1 or 2 therefore, attNumber1 and attAmount1 must be more than 0 [R56] ')).toHaveText('typeCode is equal to 1 and attType is equal to 2 and attActionType1 is equal to 1 or 2 therefore, attNumber1 and attAmount1 must be more than 0 [R56]');
    await page.locator('a').filter({ hasText: 'Batch' }).click();
